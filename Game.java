...
public class Game {
    ...
    private Place first;
    private int comLinePlayer;
    // number of points and the characters name
    private static Vector<Character> exitedPlayers = new Vector<Character>();
    IO print = new IO();
    
    public static void addExitedPlayer(Character p){
      exitedPlayers.add(p);
    }

    // public constructor
    public Game(Scanner s, int totalPlayer){
        comLinePlayer = totalPlayer;
        ...
        print.display("Game Name : " + gameName);
        print.display("Game Version : " +gameVersion);
        ...
        print.display ("The magic coin got thrown in : " + temp.name());
        Place.printAll();
    }
    ...
    public void play(){
        ...
        // adding ccharacters to the game based on the command line
        if( Game.numPlayerCharacters< comLinePlayer){
            
            
            //number of character to create
            int numPlayers = comLinePlayer-numPlayerCharacters;
            print.display("Number of Players: " + numPlayers);
            int i=0;
            ...
            while(i<numPlayers){
                ...
                print.display("Enter a name for the player "+(i+1));
                ...
                name = input.next();
                print.display("please enter a description for "+name);
                input = new Scanner(System.in);
                while(!input.hasNext()){
                    print.display("please enter a description for "+name);
                    input = new Scanner(System.in);
        ...
        if(Game.numPlayerCharacters<=0 ){
            print.display("Please enter the number of players in the game");
            Scanner input = new Scanner(System.in);
            ...
            print.display("Number of Players To Create: " + numPlayers );
            int i=0;
        ...
        print.display("All players exited the game...GAME OVER");

        int max = 0 ;
        Character winner = null; 

        for (Character c : exitedPlayers){
          if (c.returnPoints() > max) {
            max = c.returnPoints();
            winner = c;
          }
          // tie breaker
          else if (c.returnPoints() == max) {
            if (winner.returnInventoryValue() < c.returnInventoryValue()){
              max = c.returnPoints();
              winner = c;
            }
          }
        ...
        print.display("THE WINNER OF THE GAME.....IS.........");
        for (int i = 0; i<10; i++) print.display(".");
        print.display(winner.name().toUpperCase() + " WITH " + max + " POINTS");
        for (int i = 0; i<3; i++) print.display(".");
        print.display("CONGRATS " + winner.name().toUpperCase());
